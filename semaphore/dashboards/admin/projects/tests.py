...
from semaphore.dashboards.admin.projects import workflows
from semaphore.test import helpers as test
...
class UpdateProjectWorkflowTests(test.BaseAdminViewTests):
    ...
    def test_update_project_save(self):
        ...
        api.keystone.tenant_update(
            IsA(http.HttpRequest), project.id, **updated_project
        ).AndReturn(project)
        ...
        api.keystone.roles_for_user(
            IsA(http.HttpRequest), '1', self.tenant.id
        ).AndReturn(roles)

        # member user 1 - has role 1, will remove it
        api.keystone.roles_for_user(
            IsA(http.HttpRequest), '2', self.tenant.id
        ).AndReturn((roles[0],))
        # remove role 1
        ...
        api.keystone.roles_for_user(
            IsA(http.HttpRequest), '3', self.tenant.id
        ).AndReturn((roles[1],))
        # remove role 2
        api.keystone.remove_tenant_user_role(IsA(http.HttpRequest),
                                             project=self.tenant.id,
        ...
        api.keystone.roles_for_group(
            IsA(http.HttpRequest), group='1', project=self.tenant.id
        ).AndReturn(roles)
        for role in roles:
        ...
        api.keystone.roles_for_group(
            IsA(http.HttpRequest), group='2', project=self.tenant.id
        ).AndReturn((roles[0],))
        # remove role 1
        api.keystone.remove_group_role(IsA(http.HttpRequest),
                                       role='1',
        ...
        api.keystone.roles_for_group(
            IsA(http.HttpRequest), group='3', project=self.tenant.id
        ).AndReturn((roles[1],))
        # remove role 2
        api.keystone.remove_group_role(IsA(http.HttpRequest),
                                       role='2',
    ...
    def test_update_project_get_error(self):
        ...
        api.keystone.tenant_get(
            IsA(http.HttpRequest), self.tenant.id, admin=True
        ).AndRaise(self.exceptions.keystone)
    ...
    @test.create_stubs({api.keystone: ('tenant_get',
                                       ...
                                       'group_list',
                                       'role_list')})
    def test_update_project_update(self):
        project = self.tenants.first()
        default_role = self.roles.first()
        domain_id = project.domain_id
        users = self._get_all_users(domain_id)
